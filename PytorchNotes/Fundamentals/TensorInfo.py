...
add_tensor = tensor.add(torch.tensor([10, 10, 10]).to(device))
print(add_tensor)
...
print('-' * 32)
matrix_A = torch.rand(3, 5).to(device)
matrix_B = torch.rand(5, 2).to(device)
# tensor's multiplication by @ operator -> cross multiplication
...
result = torch.zeros(size=(len(matrix_A), len(matrix_B[0]))).to(device)
...
# double transpose for matrix_B, the parameters is origin dimension and target dimension
print(torch.mm(matrix_A, torch.transpose(matrix_B.T, 1, 0)))

# test torch.nn.linear for matrix_A
print("-" * 32)
input_tensor = torch.tensor([[1, 2], [3, 4], [5, 6]], dtype=torch.float).to(device)
linear = torch.nn.Linear(in_features=2, out_features=5).to(device)
# In fact, this operation is same as (3, 2) @ (2, 5), result would return shape of (3, 5)
